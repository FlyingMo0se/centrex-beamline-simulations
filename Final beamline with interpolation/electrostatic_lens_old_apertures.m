...
dz = 0.001; %meters
N_molecules = 1e0; %number of molecules for simulation
...
n_elements = n_elements + 1;
i_lens_start = n_elements; %save the index of the lens start so can study the velocity distributions before and after lens
i_lens_end = i_lens_start + round(lens.L/dz); %same for end of lens
...
%Save total number of entries needed for a molecule that goes through the
%entire beamline
i_total = n_elements + round(lens.L/dz);
...
%axis (need to check if this part is working robustly)
dr = lens.d_1/2/1e4;
...
for n_loop = 1:N_loops
    ...
    for i = 1:N_molecules/N_loops
        ...
        if alive == 0
            n_dead = n_dead + 1; %Count number of molecules that didn't make it to detection
            n_fpa = n_fpa + 1;
            x_fpa{n_fpa} = x(:,1:n); %Store trajectory
            continue %If molecule is dead, start with next molecule
        end
