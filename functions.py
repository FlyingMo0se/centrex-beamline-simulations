...
#%%
def trajectories_function(zone_of_freezing, beamline_geometry, lens,
                          beam, molecule, N_molecules,store_trajectories):
    #%% Set up parameters for the simulation
    #Set gravitational acceleration
    g = 9.81 # m/s**2
    
    #Step size for the RK4 integration inside the lens
    dz = 0.001 # meters
    
    #Calculate the acceleration inside the lens as a function of radius from
    #the centre axis by using a function called 'acceleration_table'
    dr = lens['d_1']/2/1e3
    a_values, r_values = acceleration_table(lens['V'],lens['d_1']/2,dr,
                                            molecule['J'],molecule['m'])
    ...
    number_of_points = (int(np.round(2*np.size(beamline_geometry) 
                                     + np.rint(lens['L']/dz) + 100 )))
    #Then initialize
        ...
        for i in range(0,size):
            ...
            for j in beamline_geometry:
                ...
                if not alive:
                    ...
                    if name == 'lens' or name == 'field_plate' or name =='detection_region':
    ...
    return trajectories, counts, r_values, a_values
#%% 
