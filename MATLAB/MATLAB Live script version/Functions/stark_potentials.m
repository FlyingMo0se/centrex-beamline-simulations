...
%(based on code by Adam West) (Note: as is the code is pretty inefficient,
%should change so that Hamiltonian matrix is built just once and then
%multiply relevant bits by E as in Dave's code)
...
for i=0:Jmax
    ...
    Ms=[Ms -i:i];                       % Compute lis[t of M values for the Hamiltonian basis
end
...
for n=1:length(Evals)
    ...
    for i=1:msize           
        for j=1:msize
            ...
            else                        % All other elements are zeroJ
                H(i,j)=0;
            end
        end
    end
