%Program for simulating trajectories of TlF molecules in CeNTREX from after the
...
function [n_alive, n_fp, numbers_of_hits] = electrostatic_lens_function_no_source(x_ini,v_ini,lens_length,lens_radius,electrode_radius,source_to_lens_distance,lens_voltage, J,N_molecules,plot_option)
tic
...
for i = 1:N_molecules
    ...
    alive = 1; %Number to describe if molecule is still alive (1=alive,0=dead)
...
if N_molecules <= 1e6 && plot_option == true
    ...
    xlabel('z / m')
    ylabel('x / m')
    headline = strcat('Trajectories, L = ',num2str(lens.L),' m');
    title(headline)
    hold off
    ...
    ylabel('y / m')
    hold off
    saveas(gcf,['../Plots/temp trajectories L = ' num2str(lens.L) '.jpg']);
...
%     saveas(gcf,['../Plots/temp acceleration L = ' num2str(lens_length) '.jpg']);
    ...
    %Plot the radial velocity distributions before and after the lens:
