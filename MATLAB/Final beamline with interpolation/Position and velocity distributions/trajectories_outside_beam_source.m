...
x_out_of_beam_source = zeros(N_trajectories,3);
v_out_of_beam_source = zeros(N_trajectories,3);
...
while n_out_of_beam_source <= N_trajectories
    ...
    for i = 1:N_random
        ...
        elseif alive == 1 %If molecule makes it out of beam source, store its trajectory
            ...
            x_out_of_beam_source(n_out_of_beam_source,:) = x(:,n);
            v_out_of_beam_source(n_out_of_beam_source,:) = v(:,n);
            n_end = n;
        end
        
        %If the correct number of trajectories has been found, exit for
        %loop
        
        
        if rem(n_out_of_beam_source,display_interval) == 0
            message = ['Number of trajectories found so far: ', num2str(n_out_of_beam_source)];
            disp(message)
            toc
        end
        if n_out_of_beam_source == N_trajectories
            break
        end
    end
    if n_out_of_beam_source == N_trajectories
...
save('velocities_after_beamsource.mat','v_final')
save('positions_after_beam_source.mat','x_final')
