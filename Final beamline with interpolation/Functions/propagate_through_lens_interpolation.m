...
function [x,v,alive,n] = propagate_through_lens_interpolation(x,v,a_values,r_values,lens,molecule,n,alive,option_1,g)
    ...
    if lens.z_2-lens.z_1 > 0 && alive
        ...
        for n = n:(n + N_steps)
            if alive == 1 %if molecule is alive update position and velocity
                %Calculate the parameters for RK
                k_1 = v(:,n);
                l_1 = acceleration_interpolation(x(:,n),a_values,r_values,g);
                k_2 = v(:,n)+dt*l_1/2;
                l_2 = acceleration_interpolation(x(:,n)+dt*k_1/2,a_values,r_values,g);
                k_3 = v(:,n)+dt*l_2/2;
                l_3 = acceleration_interpolation(x(:,n)+dt*k_2/2,a_values,r_values,g);
                k_4 = v(:,n)+dt*l_3;
                l_4 = acceleration_interpolation(x(:,n)+dt*k_3,a_values,r_values,g);
